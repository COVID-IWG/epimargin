...
if __name__ == "__main__":
    ...
    plot_simulation_range(simulation_results, ["31 May Release", "Adaptive Control"], historical=state_ts["Hospitalized"], smoothing = smoothed)\
        .title("Bihar Policy Scenarios: Projected Infections (0% Migration Influx on 24 May)")\
        .xlabel("Date")\
        .ylabel("Number of Infections")\
        .annotate(f"data from May 20 | stochastic parameter range: ({si}, {sf}) | infectious period: {1/gamma} days | smoothing window: {window} | release rate: 0% | number of migrants {num_migrants} ")
    plt.show()
    ...
    projdf.to_csv(figs/"bihar_may20_rt.csv")
