...
def deconv(observed, kernel): 
    ...
    def error(x):
        return sum(convolve(x, kernel, mode = "full")[:len(x)] - padded) ** 2
...
pmf  = gamma.pdf(np.linspace(gamma.ppf(0.005, a), gamma.ppf(1-0.005, a)), a)/sum(pmf)
...
t = np.linspace(0, 150)
S = 2000 * logistic.cdf((t - 75)/10)
g = gamma.pdf(np.linspace(gamma.ppf(0.005, a), gamma.ppf(1-0.005, a)), a)
# f = np.ones(5)/5
# S = convolve(I, f)

# plt.plot(t, S[-len(t):], color = color)
