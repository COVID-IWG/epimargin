...
try: 
    for _ in range(1 * 365):
        ...
        dV = multinomial.rvs(
            phi * (S0 + I0 + R0), 
            normalize(random_vax_model.N[-1], axis = 1)[0])\
            .reshape((-1, num_sims))
        dVs.append(dV)
...
print(seed, "random D", random_vax_model.D[-1].sum())
print(seed, "no vax D",     no_vax_model.D[-1].sum())
print(seed, "random D since vax start", (random_vax_model.D[-1] - random_vax_model.D[0]).sum())
print(seed, "no vax D since vax start", (    no_vax_model.D[-1] -     no_vax_model.D[0]).sum())

print(seed, "random D", random_vax_model.D[-1])
print(seed, "no vax D",     no_vax_model.D[-1])
print(seed, "random D since vax start", random_vax_model.D[-1] - random_vax_model.D[0])
print(seed, "no vax D since vax start",     no_vax_model.D[-1] -     no_vax_model.D[0])

print(seed, "random q1", random_vax_model.q1[-1])
print(seed, "random q0", random_vax_model.q0[-1])
print(seed, "no vax q0", no_vax_model.q0[-1])
print(seed, "sign(dq0)", np.sign(random_vax_model.q0[-1] - no_vax_model.q0[-1]))
