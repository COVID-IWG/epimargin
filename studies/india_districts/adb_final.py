...
ts = get_time_series(df, "detected_state").query("status_change_date <= 'October 14, 2020'")

states = ["Bihar", "Maharashtra", "Punjab", "Tamil Nadu"]
codes  = ["BR",    "MH",          "PN",     "TN"]
pops   = [99.02e6, 114.2e6,       27.98e6,  67.86e6]


for (state, code, pop) in zip(states, codes, pops): 
    print(state)
    ...
    state_model = SIR(name = state, population = pop, dT0 = T_pred[-1], Rt0 = Rt_pred[-1], mobility = 0, I0 = I0, upper_CI = T_CI_upper[-1], lower_CI = T_CI_lower[-1], random_seed = 0).run(14)

    plt.daily_cases(dates, T_pred, T_CI_upper, T_CI_lower, new_cases_ts, anomaly_dates, anomalies, CI, 
        prediction_ts=[(state_model.dT, state_model.lower_CI, state_model.upper_CI, plt.PRED_PURPLE, "predicted cases")])\
        .ylabel("cases")\
        .xlabel("date")\
        .title(f"\n{state}: Daily Cases")\
    ...
    estimates.clip(0).to_csv(data/f"Rt_estimates_{code}.csv")
    print(estimates)
    ...
    plt.choropleth(gdf, mappable = plt.get_cmap(0.5, 2.2))\
