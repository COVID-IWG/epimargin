...
with tqdm(state_names) as states:
    for state in states:
        ...
        try: 
            (dates, RR_pred, RR_CI_upper, RR_CI_lower, T_pred, T_CI_upper, T_CI_lower, total_cases, new_cases_ts, anomalies, anomaly_dates) = gamma_prior(state_time_series.loc[state]['Hospitalized'], CI = CI, smoothing = convolution(window = smoothing))
            for row in zip(dates, RR_pred, RR_CI_upper, RR_CI_lower):
